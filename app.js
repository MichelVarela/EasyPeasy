...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			Total de peliculas en cartelera: ${movies.length}
			
			LISTADO DE PELICULAS:`)
            
			let titulos = [];//array donde guardamos los titulos
            movies.forEach(title=>{
                return titulos.push(title.original_title);//indicamos que vamos a agregar a la variable titulos lo que venga de title.original
            });

			titulos.sort();//ordenamos mediante la propiedad sort
			titulos.forEach(elemento=>{
				res.write(`
				-${elemento}\n`)//res.write para mostrar en pantalla funciona como un return para el foreach y sus iteraciones
			})
			...
			-Sucursales
			-Contacto
			-Preguntas Frecuentes`);//res.write para mostrar en pantalla
        

            res.end();//siempre ponerlo para que termine la escucha
			break;
		// En cartelera
		case '/en-cartelera':
			...
			let titulo = [];//guarda el titulo y el resumen
            movies.forEach(function(dato){
                return titulo.push(`${dato.title.toUpperCase()} : ${dato.overview}`);//pusheamos a tituloResumen y toUppercase para poner el titulo en mayus
            });
			titulo.sort();//ordenamos alfabeticamente
			titulo.forEach(elemento=>{
				res.write(`
				${elemento}\n\n`)//utilizamos res.write como return para que retorne  en pantalla cada pelicula iterada por el foreach
			})
		...
		case '/mas-votadas':
			...
			let peliculasMasVotadas = [];

			movies.filter(raiting=>{
				if(raiting.vote_average >= 7){
					return peliculasMasVotadas.push(raiting.vote_average);
				};
			});

			let promedio = peliculasMasVotadas.reduce(function(actual,numero){
				return actual + numero;
			})

			promediofinal = promedio/peliculasMasVotadas.length//luego utilizamos toFixed para redondear los decimales a solo 2 

            res.write(`MAS VOTADAS
            Total de peliculas mas votadas: ${peliculasMasVotadas.length}
            
            RAITING PROMEDIO: ${promediofinal.toFixed(2)}
            
            LISTADO DE PELICULAS:
			`);

			movies.filter(function(listado){//filtramos los votos mayores a siete
                if(listado.vote_average >= 7){ 
                    res.write(`
                    PELICULA: ${listado.title} 
                    RAITING: ${listado.vote_average} 
                    REVIEW: ${listado.overview}
                    `);//utilizamos res.write como devolucion del if que nos devuelve en el bactick pelicula, raiting y review
                }
            })
		...
		case '/sucursales':
			...
			Cantidad de Salas: ${theaters.length}
			
			LISTADO DE SALAS:
			`);

			theaters.forEach(salas => {
				res.write(`
				${salas.name}
				
				UBICACION: ${salas.address}
				
				DESCRIPCION: ${salas.description}`);//utilizamos res.write como return para foreach
			});
